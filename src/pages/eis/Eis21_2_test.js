...
export default function Ecs22_2_test({ userId = "defaultUserId" }) {
  ...
  const TEST_NAME = "산업기사 22년 2회차"; // 시험 이름
  ...
  const fetchQuestions = async () => {
    setLoading(true);
    try {
      const q = query(collection(db, 'eis21_2'), orderBy('Question_Num', 'asc'));
      ...
      const questionsData = await Promise.all(querySnapshot.docs.map(async (doc) => {
        ...
        if (typeof data.Image === 'string' && data.Image !== "Null") {
          ...
          data.Image = await getDownloadURL(imageRef).catch(() => null); // 이미지 다운로드 실패 시 null 처리
        }
  ...
  const fetchAnswers = async () => {
    try {
      const questionsQuery = query(collection(db, 'eis21_2'), orderBy('Question_Num', 'asc'));
      const questionsSnapshot = await getDocs(questionsQuery);
  
      const explanationsQuery = query(collection(db, 'eis21_2_explain'));
