...
export default function Efps21_1_test({ userId = "defaultUserId" }) { // userId 추가
  ...
  const TEST_NAME = "소방설비기사(기계) 21년 1회차"; // 시험 이름

  const formatSolvedCount = () => `${solvedCount} / 80`;

  const fetchQuestions = async () => {
    setLoading(true);
    try {
      const q = query(collection(db, 'efps21_1'), orderBy('Question_Num', 'asc'));
  ...
  const fetchAnswers = async () => {
    try {
      const questionsQuery = query(collection(db, 'efps21_1'), orderBy('Question_Num', 'asc'));
      const questionsSnapshot = await getDocs(questionsQuery);
  
      const explanationsQuery = query(collection(db, 'efps21_1_explain'));
