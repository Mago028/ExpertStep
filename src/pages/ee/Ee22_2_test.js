...
import { collection, getDocs, query, orderBy, addDoc } from "firebase/firestore"; // addDoc 추가
...
import "./Ee22_test.css";

export default function Ee22_2_test({ userId = "defaultUserId" }) { // userId 추가
  ...
  const TOTAL_SUBJECTS = 5; // 총 과목 수
  const TEST_NAME = "전기기사 22년 2회차"; // 시험 이름

  const formatSolvedCount = () => `${solvedCount} / 100`;

  const fetchQuestions = async () => {
    setLoading(true);
    try {
      const q = query(collection(db, 'ee22_2'), orderBy('Question_Num', 'asc'));
  ...
  const fetchAnswers = async () => {
    try {
      const questionsQuery = query(collection(db, 'ee22_1'), orderBy('Question_Num', 'asc'));
      const questionsSnapshot = await getDocs(questionsQuery);
  
      const explanationsQuery = query(collection(db, 'ee22_1_explain'));
  ...
  const unsolvedCount = 100 - solvedCount; // 이 부분을 추가하여 unsolvedCount를 정의합니다.
        ...
        {!loading && !error && currentQuestion && (
          <div id={`question-${currentQuestion.Question_Num}`}>
            ...
            {currentQuestion.Image && (
              <img src={currentQuestion.Image} alt="문제 이미지" style={{ maxWidth: "100%", maxHeight: "200px" }} />
            )}
