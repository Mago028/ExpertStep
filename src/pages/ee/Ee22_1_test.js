...
export default function Ee22_1_test({ userId = "defaultUserId" }) {
  const [solvedCount, setSolvedCount] = useState(0);
  const [timeLeft, setTimeLeft] = useState(9000); // 150분 = 9000초
  const [selectedAnswers, setSelectedAnswers] = useState(new Array(100).fill(null)); // 기본값 100문제
  ...
  const [solvedQuestions, setSolvedQuestions] = useState(new Array(100).fill(false)); // 기본값 100문제
  
  const navigate = useNavigate();

  const TOTAL_SUBJECTS = 5; // 총 과목 수
  const TEST_NAME = "전기기사 22년 1회차"; // 시험 이름
  ...
  const fetchQuestions = async () => {
    setLoading(true);
    try {
      const q = query(collection(db, 'ee22_1'), orderBy('Question_Num', 'asc'));
