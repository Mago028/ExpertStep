...
export default function Ecs22_1_test({ userId = "defaultUserId" }) {
  ...
  const TEST_NAME = "건설안전기사 22년 1회차"; // 시험 이름
  ...
  const fetchQuestions = async () => {
    setLoading(true);
    try {
      const q = query(collection(db, 'ecs22_1'), orderBy('Question_Num', 'asc'));
  ...
  const fetchAnswers = async () => {
    try {
      const questionsQuery = query(collection(db, 'ecs22_1'), orderBy('Question_Num', 'asc'));
      const questionsSnapshot = await getDocs(questionsQuery);
  
      const explanationsQuery = query(collection(db, 'ecs22_1_explain'));
