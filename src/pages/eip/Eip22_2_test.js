...
import { collection, getDocs, query, orderBy, limit, addDoc } from "firebase/firestore"; // addDoc 추가
...
import "./Eip22_test.css";

// 컴포넌트 시작 부분에서 userId를 prop으로 받도록 수정
export default function Eip22_2_test({ userId = "defaultUserId" }) { // userId 추가

  const [solvedCount, setSolvedCount] = useState(0);
  const [timeLeft, setTimeLeft] = useState(9000); // 2시간 30분 = 9000초
  const [selectedAnswer, setSelectedAnswer] = useState(null);
  const [selectedAnswers, setSelectedAnswers] = useState(new Array(100).fill(null));
  ...
  const [solvedQuestions, setSolvedQuestions] = useState(new Array(100).fill(false));

  const navigate = useNavigate();

  const TOTAL_SUBJECTS = 6; // 총 과목 수
  const TEST_NAME = "정보처리기사 22년 2회차"; // 시험 이름

  const formatSolvedCount = () => {
    return `${solvedCount} / 100`;
  };

  const unsolvedCount = 100 - solvedCount;

  const fetchInitialQuestion = async () => {
    setLoading(true);
    try {
      const q = query(collection(db, "eip22_2"), orderBy("Question_Num", "asc"), limit(1));
  ...
  const fetchAllQuestions = async () => {
    try {
      const q = query(collection(db, "eip22_2"), orderBy("Question_Num", "asc"));
      const querySnapshot = await getDocs(q);
      const questionsData = await Promise.all(querySnapshot.docs.map(async (doc) => {
        const data = doc.data();
        if (typeof data.Image === "string" && data.Image !== "Null") {
  ...
  const fetchAnswers = async () => {
    try {
      const questionsQuery = query(collection(db, 'eip22_2'), orderBy('Question_Num', 'asc'));
      const questionsSnapshot = await getDocs(questionsQuery);
  
      const explanationsQuery = query(collection(db, 'eip22_2_explain'));
  ...
  const scrollToQuestion = (questionNumber) => {
    ...
    if (questionIndex !== -1) {
      ...
      setSelectedAnswer(null); // 이전 선택 취소
    }
  };
      ...
      <div className="Question-sheet">
        ...
        {!loading && !error && currentQuestion && (
          <div id={`question-${currentQuestion.Question_Num}`}>
            ...
            {selectedAnswer && currentQuestionIndex !== 99 && (
      ...
      <div>
        {isSubmitPopupOpen && (
          <div className="Submit-popup">
            ...
            <div className="popup-submit-description">
              제출 후에는 수정이 불가합니다.
            </div>
