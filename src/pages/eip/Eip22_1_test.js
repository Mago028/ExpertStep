...
export default function Eip22_1_test({ userId = "defaultUserId" }) {
  ...
  const TEST_NAME = "정보처리기사 22년 1회차"; // 시험 이름
  ...
  const fetchInitialQuestion = async () => {
    setLoading(true);
    try {
      const q = query(collection(db, 'eip22_1'), orderBy('Question_Num', 'asc'), limit(1));
  ...
  const fetchAllQuestions = async () => {
    try {
      const q = query(collection(db, 'eip22_1'), orderBy('Question_Num', 'asc'));
      const querySnapshot = await getDocs(q);
      const questionsData = await Promise.all(querySnapshot.docs.map(async (doc) => {
        const data = doc.data();
        if (typeof data.Image === 'string' && data.Image !== "Null") {
  ...
  const fetchAnswers = async () => {
    try {
      const questionsQuery = query(collection(db, 'eip22_1'), orderBy('Question_Num', 'asc'));
      const questionsSnapshot = await getDocs(questionsQuery);

      const explanationsQuery = query(collection(db, 'eip22_1_explain'));
